...
try {
  ...
  # [UI-04] A real pointer hover must expose the shared blue action cue.
  Invoke-MuseumBrowser run-code "async (page) => { const button=page.locator('#btn-reset');await button.hover();const color=await button.evaluate(function(element){return getComputedStyle(element).color;});if(color!=='rgb(102, 217, 239)')throw new Error('graph control hover color missing: '+color); }"
  Invoke-MuseumBrowser eval "() => { const data=JSON.parse(document.querySelector('#graph-data').textContent);const expected=data.nodes.filter(function(node){return (node.degree||0)===0;});if(!expected.length)return 'isolated-node-fixture-empty';const rendered=Array.from(document.querySelectorAll('.graph-nodes g.is-isolated'));if(rendered.length!==expected.length)throw new Error('rendered '+rendered.length+' isolated nodes instead of '+expected.length);const fallback=document.querySelector('#graph-isolated-fallback');if(!fallback||fallback.hidden)throw new Error('isolated note list is hidden');const rows=fallback.querySelectorAll('#graph-isolated-list article');if(rows.length!==expected.length)throw new Error('isolated list has '+rows.length+' rows instead of '+expected.length);const dot=rendered[0].querySelector('.graph-node-dot');if(!dot||getComputedStyle(dot).strokeDasharray==='none')throw new Error('isolated node has no distinct visual treatment');return 'isolated-nodes-ok:'+expected.length; }"
